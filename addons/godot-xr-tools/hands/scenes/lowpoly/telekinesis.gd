...
func _process(_delta):
	...
	var new_at : Vector3
	var suppress_area := $FunctionPointer/SuppressArea
	if (enabled and
		not $FunctionPointer/SuppressArea.has_overlapping_bodies() and
		not $FunctionPointer/SuppressArea.has_overlapping_areas() and
		$FunctionPointer/RayCast.is_colliding()):
		new_at = $FunctionPointer/RayCast.get_collision_point()
		if target:
			# Locked to 'target' even if we're colliding with something else
			new_target = target
		else:
			# Target is whatever the raycast is colliding with
			new_target = $FunctionPointer/RayCast.get_collider()
...
func _update_y_offset() -> void:
	$FunctionPointer/Laser.position.y = y_offset * _world_scale
	$FunctionPointer/RayCast.position.y = y_offset * _world_scale


# Pointer distance update handler
func _update_distance() -> void:
	$FunctionPointer/RayCast.target_position.z = -distance
	_update_pointer()


# Pointer target radius update handler
func _update_target_radius() -> void:
	$FunctionPointer/Target.mesh.radius = target_radius
	$FunctionPointer/Target.mesh.height = target_radius * 2


# Pointer target_material update handler
func _update_target_material() -> void:
	$FunctionPointer/Target.set_surface_override_material(0, target_material)


# Pointer collision_mask update handler
func _update_collision_mask() -> void:
	$FunctionPointer/RayCast.collision_mask = collision_mask


# Pointer collide_with_bodies update handler
func _update_collide_with_bodies() -> void:
	$FunctionPointer/RayCast.collide_with_bodies = collide_with_bodies


# Pointer collide_with_areas update handler
func _update_collide_with_areas() -> void:
	$FunctionPointer/RayCast.collide_with_areas = collide_with_areas


# Pointer suppress_radius update handler
func _update_suppress_radius() -> void:
	$FunctionPointer/SuppressArea/CollisionShape3D.shape.radius = suppress_radius


# Pointer suppress_mask update handler
func _update_suppress_mask() -> void:
	$FunctionPointer/SuppressArea.collision_mask = suppress_mask
...
func _update_laser_active_material(hit : bool) -> void:
	if hit and laser_hit_material:
		$FunctionPointer/Laser.set_surface_override_material(0, laser_hit_material)
	else:
		$FunctionPointer/Laser.set_surface_override_material(0, laser_material)
...
func _visible_hit(at : Vector3) -> void:
	...
	if show_target:
		$FunctionPointer/Target.global_transform.origin = at
		$FunctionPointer/Target.visible = true
	...
	if show_laser != LaserShow.HIDE:
		...
		if laser_length == LaserLength.COLLIDE:
			...
			$FunctionPointer/Laser.mesh.size.z = collide_len
			$FunctionPointer/Laser.position.z = collide_len * -0.5
		else:
			$FunctionPointer/Laser.mesh.size.z = distance
			$FunctionPointer/Laser.position.z = distance * -0.5

		# Show laser
		$FunctionPointer/Laser.visible = true
	else:
		# Ensure laser is hidden
		$FunctionPointer/Laser.visible = false
...
func _visible_miss() -> void:
	...
	$FunctionPointer/Target.visible = false

	# Ensure the correct laser material is set
	_update_laser_active_material(false)

	# Hide laser if not set to show always
	$FunctionPointer/Laser.visible = show_laser == LaserShow.SHOW
